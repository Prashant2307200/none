...
# ---------------------------------------------------------------------------
# SECURITY GENERATOR: real logon/logoff events are slow (Start-Process per
# event), so they run on their own cadence instead of blocking the fast
# System/Application/WinLogGen path that carries the bulk of the volume.
# ---------------------------------------------------------------------------
function Start-SecuritySimulator {
    param([string]$TestUser, [string]$StopFlag)
    $job = Start-Job -Name 'LogSimSecurity' -ScriptBlock {
        param($TestUser, $StopFlag)
    ...
    } -ArgumentList $TestUser, $StopFlag
    return $job
}
...
function Invoke-Run {
    ...
    $securityJob = Start-SecuritySimulator -TestUser $TestUser -StopFlag $StopFlag

    $targetBytesPerDay = $TargetGBPerDay * 1GB
    # Security runs on its own slow background loop (real auth round-trips,
    # ~150-300ms each) and isn't part of the bulk-volume budget below.
    $weights = @{ System = 0.25; Application = 0.40; WinLogGen = 0.35 }
    $avgEventBytes = 1700
    ...
    while ($true) {
        ...
        $eventsThisTick = [math]::Max(1, [int]($baseEventsPerSec * $mult * $tickSeconds))
        ...
        # Pace to ~1s ticks only if we finished early; never oversleep past it,
        # and never block waiting if generation itself is the bottleneck.
        $elapsedMs = $tickWatch.Elapsed.TotalMilliseconds
        if ($elapsedMs -lt 1000) { Start-Sleep -Milliseconds ([int](1000 - $elapsedMs)) }
        $tickSeconds = $tickWatch.Elapsed.TotalSeconds
    }

    Stop-Job $archiverJob, $securityJob -ErrorAction SilentlyContinue
    Remove-Job $archiverJob, $securityJob -ErrorAction SilentlyContinue
